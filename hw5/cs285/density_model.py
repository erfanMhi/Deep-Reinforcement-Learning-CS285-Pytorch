...
class RBF(Density_Model):
    ...
    def get_prob(self, states):
        ...
        if self.means is None:
            ...
            # 1. Compute deltas
            deltas = np.array([[s - rb_s for s in self.means] for s in rb_s])
            assert deltas.shape == (b, B, ob_dim)

            # 2. Euclidean distance
            euc_dists = np.sqrt(np.power(deltas, 2)).sum(2)
...
class Exemplar(Density_Model):
    ...
    def build_computation_graph(self):
        ...
        kl_diverge = tfp.distributions.kl_divergence
        self.kl = kl_diverge(self.encoder1, self.prior) + kl_diverge(self.encoder2, self.prior)
    ...
    def make_prior(self, z_size):
        ...
        """
        prior_mean = tf.Variable(tf.zeros(z_size), dtype=np.float32)
        prior_logstd = tf.Variable(tf.zeros(z_size), dtype=np.float32)
    ...
    def get_likelihood(self, state1, state2):
        ...
        likelihood = self.sess.run(self.likelihood,
                     feed_dict={self.state1: state1, self.state2: state2, self.discrim_target: np.ones((state1.shape[0], 1))}) #TODO: why ones
